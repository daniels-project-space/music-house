  ...
  return (
    <main className="min-h-screen pb-32">
      ...
      <div className="relative">
        ...
        <div className="px-6 sm:px-10 lg:px-16 pt-16 pb-10 max-w-6xl mx-auto flex flex-col sm:flex-row gap-8 items-center sm:items-end">
          <div
            className="w-56 h-56 sm:w-64 sm:h-64 rounded-lg overflow-hidden ring-1 ring-paper/10 bg-paper/[0.04] shrink-0"
          ...
          <div className="flex-1 min-w-0 text-center sm:text-left">
            ...
            <h1 className="font-display text-3xl sm:text-5xl font-bold tracking-tight text-paper leading-tight mb-3">
              {albumRow.name}
            </h1>
            <p className="font-display text-[1rem] text-paper-dim mb-1">
              {artistRow?.name ?? albumRow.artistSlug}
            </p>
            <p className="font-mono text-[0.6rem] uppercase tracking-[0.18em] text-paper-faint">
              {sorted.length} tracks
              {albumRow.genre ? ` · ${albumRow.genre}` : ""}
            </p>
            {albumRow.description ? (
              <p className="font-display text-[0.85rem] text-paper-dim mt-4 max-w-xl">{albumRow.description}</p>
            ) : null}
          </div>
        </div>
      </div>
      ...
      {/* Tracklist */}
      <div className="px-6 sm:px-10 lg:px-16 max-w-4xl mx-auto mt-2">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-display text-[0.92rem] text-paper truncate leading-tight">{t.title}</div>
                  {t.genre ? (
                    <div className="font-mono text-[0.55rem] uppercase tracking-[0.14em] text-paper-faint mt-0.5">{t.genre}</div>
                  ) : null}
                </div>
                <span className="font-mono text-[0.7rem] text-paper-faint tabular-nums">
                  {dura ? fmt(dura) : "—"}
                </span>
              </li>
            );
      ...
      {activeTrack ? (
        ...
        >
          <div className="px-6 sm:px-10 lg:px-16 max-w-4xl mx-auto py-3 flex items-center gap-4">
            <button
              onClick={() => playTrack(activeTrack)}
              className="w-10 h-10 rounded-full grid place-items-center text-paper"
            ...
            <div className="min-w-0 flex-1">
              <div className="font-display text-[0.85rem] text-paper truncate leading-tight">{activeTrack.title}</div>
              <div className="font-mono text-[0.55rem] uppercase tracking-[0.14em] text-paper-faint truncate">
                {artistRow?.name ?? albumRow.artistSlug}
              </div>
            </div>
